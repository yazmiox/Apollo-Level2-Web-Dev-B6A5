    ...
    try {
      ...
      if (needsImageUpload) {
        toast.loading(isEditMode ? "Updating equipment..." : "Adding equipment...", {
          id: toastId,
          description: "Preparing image upload..."
        });
      ...
      if (!res.success) throw new Error(res.message || "Failed to save equipment");
      ...
      <div className="relative w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl animate-in zoom-in-95 fade-in duration-200 h-fit">
        ...
        <form className="space-y-8" onSubmit={handleSubmit}>
          ...
          <div className="space-y-3">
            ...
            {!preview ? (
              <label onClick={() => fileInputRef.current?.click()} className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-[#e0dbd3] bg-[#f9f8f6] py-12 cursor-pointer transition-colors hover:border-[#e8612e]/40 hover:bg-[#e8612e]/5">
                <ImageIcon size={32} className="mb-2 text-[#ccc]" />
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            <div className="flex flex-col gap-1.5">
              ...
              <input name="name" type="text" defaultValue={initialData?.name} required placeholder="Sony FX6..." className="rounded-[7px] border border-[#e0dbd3] px-3 py-2 text-sm text-[#111] outline-none focus:border-[#e8612e] focus:ring-1 focus:ring-[#e8612e] transition-all" />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[11px] font-bold uppercase tracking-wider text-[#777]">Category</label>
              <select name="categoryId" defaultValue={initialData?.categoryId} className="rounded-[7px] border border-[#e0dbd3] px-3 py-2 text-sm text-[#111] outline-none focus:border-[#e8612e] focus:ring-1 focus:ring-[#e8612e] transition-all">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input name="brand" type="text" defaultValue={initialData?.brand} placeholder="Sony, Canon..." className="rounded-[7px] border border-[#e0dbd3] px-3 py-2 text-sm text-[#111] outline-none focus:border-[#e8612e] focus:ring-1 focus:ring-[#e8612e] transition-all" />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[11px] font-bold uppercase tracking-wider text-[#777]">Model</label>
              <input name="modelName" type="text" defaultValue={initialData?.modelName} placeholder="FX6, EOS R5..." className="rounded-[7px] border border-[#e0dbd3] px-3 py-2 text-sm text-[#111] outline-none focus:border-[#e8612e] focus:ring-1 focus:ring-[#e8612e] transition-all" />
            </div>

            <div className="flex flex-col gap-1.5 sm:col-span-2">
              <label className="text-[11px] font-bold uppercase tracking-wider text-[#777]">Long Description</label>
              <textarea name="description" defaultValue={initialData?.description} rows={4} placeholder="Detailed equipment information..." className="rounded-[7px] border border-[#e0dbd3] px-3 py-2 text-sm text-[#111] outline-none focus:border-[#e8612e] focus:ring-1 focus:ring-[#e8612e] transition-all"></textarea>
            </div>

            <div className="flex flex-col gap-1.5">
              <label className="text-[11px] font-bold uppercase tracking-wider text-[#777]">Pickup Location</label>
              <input name="location" type="text" defaultValue={initialData?.location} placeholder="Studio A, Storage B..." className="rounded-[7px] border border-[#e0dbd3] px-3 py-2 text-sm text-[#111] outline-none focus:border-[#e8612e] focus:ring-1 focus:ring-[#e8612e] transition-all" />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[11px] font-bold uppercase tracking-wider text-[#777]">Condition</label>
              <select name="condition" defaultValue={initialData?.condition} className="rounded-[7px] border border-[#e0dbd3] px-3 py-2 text-sm text-[#111] outline-none focus:border-[#e8612e] focus:ring-1 focus:ring-[#e8612e] transition-all">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select name="status" defaultValue={initialData?.status} className="rounded-[7px] border border-[#e0dbd3] px-3 py-2 text-sm text-[#111] outline-none focus:border-[#e8612e] focus:ring-1 focus:ring-[#e8612e] transition-all">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input name="rentalRate" type="number" defaultValue={initialData?.rentalRate} min="0" step="0.01" placeholder="85.00" required className="rounded-[7px] border border-[#e0dbd3] px-3 py-2 text-sm text-[#111] outline-none focus:border-[#e8612e] focus:ring-1 focus:ring-[#e8612e] transition-all" />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[11px] font-bold uppercase tracking-wider text-[#777]">What's Included (Comma separated)</label>
              <input name="includedItems" type="text" defaultValue={initialData?.includedItems?.join(", ")} placeholder="Battery, Charger, SD Card..." className="rounded-[7px] border border-[#e0dbd3] px-3 py-2 text-sm text-[#111] outline-none focus:border-[#e8612e] focus:ring-1 focus:ring-[#e8612e] transition-all" />
            </div>
          </div>
            ...
            <div className="grid gap-3">
              {specifications.map((spec) => (
                <div key={spec.id} className="flex gap-2 animate-in slide-in-from-top-1 duration-200">
                  <input type="text" placeholder="Title (e.g. Sensor)" value={spec.key} onChange={(e) => updateSpecification(spec.id, "key", e.target.value)} className="flex-1 rounded-[7px] border border-[#e0dbd3] px-3 py-2 text-sm text-[#111] outline-none focus:border-[#e8612e] transition-all" />
                  <input type="text" placeholder="Value (e.g. Micro P3)" value={spec.value} onChange={(e) => updateSpecification(spec.id, "value", e.target.value)} className="flex-1 rounded-[7px] border border-[#e0dbd3] px-3 py-2 text-sm text-[#111] outline-none focus:border-[#e8612e] transition-all" />
                  <button type="button" onClick={() => removeSpecification(spec.id)} className="rounded-md border border-[#e0dbd3] p-2 text-[#aaa] hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-all">
                    <Trash2 size={16} />
                  </button>
                </div>
              ))}
              {specifications.length === 0 && (
                <p className="text-xs text-[#aaa] italic text-center py-6 border border-dashed border-[#e0dbd3] rounded-xl bg-[#f9f8f6]">
